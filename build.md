...
> **Carried into Sprint 4 (not blockers for the M1 deliverable):** passkey submit-path integration in `@kaido/sdk` — the SDK always submits via RPC `sendTransaction`; passkey-signed smart-wallet txs need to go through the `/api/launchtube` route instead (the connector signs correctly today, but the SDK's `signAndSend` path doesn't yet route to Launchtube), so passkey play needs this before it's end-to-end. Playwright ChartGuessr happy path needs a seeded tight-window test market + a mocked clock. **Operational note:** `config/networks.testnet.json` on disk still holds the Sprint-1 (scaffold) ids — the live `/markets`, `/create` and ChartGuessr pages only return data after a `make deploy:testnet` re-deploy with `USDC_SAC_ID` / `REFLECTOR_FEED_ID` set in `.env` so `external.*` and `demo.*` get populated.

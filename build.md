...
> **Not done yet (next session):** `@kaido/sdk` write path (`createMarket`/`trade`/`addLiquidity`/`resolve` + a pluggable signer); passkey-kit + Launchtube wallet + Freighter fallback; `web/lib/curve` (deterministic path→checkpoint values, sharing `docs/test-vectors/` with `kaido-math`); the ChartGuessr loop (45s/15s/reveal, play-vs-house, live BTC via Reflector RPC reads); `/create` wizard wiring; `resolver-reflector` trajectory mode (its `__constructor` is single-checkpoint — trajectory markets currently need a vec-outcome resolver supplied externally); a `deploy.sh` trajectory demo. **Operational note:** `config/networks.testnet.json` on disk still holds the Sprint-1 (scaffold) factory/registry ids — the live `/markets` pages only return data after a `make deploy:testnet` re-deploy.

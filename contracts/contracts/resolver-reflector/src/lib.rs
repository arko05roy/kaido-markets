//! Resolver T0 — reads a Reflector SEP-40 price feed (TWAP-style for short windows).
//!
...
mod test {
    ...
    fn scaffold_version_is_zero() {
        let env = Env::default();
        let id = env.register(ResolverReflector, ());
        let client = ResolverReflectorClient::new(&env, &id);
